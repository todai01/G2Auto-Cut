...
class VariablesMixin:
    ...
    # ==================================================================
    #  ПОДРАВНИВАНИЕ ПАУЗ ПО ЭТАЛОНУ
    #  Та же логика «эталон → применить ко всем», что и у нормализации
    #  громкости, но переносится не абсолютное смещение (оно у каждого
    #  числа своё — «1 миллион» и «99 миллионов» звучат по-разному долго),
    #  а ПРАВИЛО: сколько тишины оставлять до и после речи. У каждого
    #  файла это правило применяется к его же собственным границам речи —
    #  поэтому не важно, что файлы разной длины.
    # ==================================================================

    @staticmethod
    def _speech_bounds_ms(audio):
        """Начало и конец речи в файле (мс) — по тишине относительно
        собственной громкости этого же файла, а не общего порога."""
        thresh = min(audio.dBFS - 14, -30)
        ranges = silence.detect_nonsilent(audio, min_silence_len=60, silence_thresh=thresh)
        if not ranges:
            return None
        return ranges[0][0], ranges[-1][1]

    def prepare_pause_trim(self):
        """ШАГ 1: как и в нормализации — папка, первый файл эталоном в Audacity."""
        folder = webview.windows[0].create_file_dialog(webview.FileDialog.FOLDER)
        if not folder:
            return {"error": "cancel"}
        ...
        first_file = files[0]
        self.pause_trim_files = files
        self.pause_trim_first_file = first_file
    ...
    def apply_pause_trim(self):
        """ШАГ 2: экспортирует подрезанный эталон, замеряет отступы тишины
        до/после речи и применяет то же ПРАВИЛО отступов ко всем остальным
        файлам — каждому по его собственным границам речи."""
        if not hasattr(self, 'pause_trim_files') or not self.pause_trim_files:
            return {"error": "Нет файлов для обработки"}

        temp_path = os.path.join(os.path.dirname(self.pause_trim_first_file), "temp_ref_trim.wav")
        safe_temp_path = os.path.abspath(temp_path).replace('\\', '/')
        ...
        try:
            ref_audio = AudioSegment.from_file(temp_path)
            bounds = self._speech_bounds_ms(ref_audio)
            if bounds is None:
                return {"error": "В эталоне не нашлось речи — проверьте, что не обрезали всё целиком."}

            lead_pad = bounds[0]
            trail_pad = len(ref_audio) - bounds[1]

            ref_audio.export(self.pause_trim_first_file, format="wav")

            total = len(self.pause_trim_files)
            skipped = 0
            for i, filepath in enumerate(self.pause_trim_files):
                if filepath == self.pause_trim_first_file:
                    continue
                if not os.path.exists(filepath):
                    continue

                chunk_audio = AudioSegment.from_file(filepath)
                chunk_bounds = self._speech_bounds_ms(chunk_audio)
                if chunk_bounds is None:
                    skipped += 1
                    continue

                start = max(0, chunk_bounds[0] - lead_pad)
                end = min(len(chunk_audio), chunk_bounds[1] + trail_pad)
                chunk_audio[start:end].export(filepath, format="wav")

                if i % 5 == 0:
                    pct = int((i / total) * 100)
                    try:
                        webview.windows[0].evaluate_js(
                            f"updateProgress({pct}, 'Подравнивание пауз: {i}/{total}...');")
                    except:
                        pass
            ...
            self.pause_trim_files = []
            if skipped:
                return {"status": "success", "skipped": skipped}
            return {"status": "success"}
        except Exception as e:
            return {"error": f"Ошибка подравнивания пауз: {str(e)}"}
